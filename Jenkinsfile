    ...
    stage('Validate Metrics') {
      steps {
        sh '''
          ...
          curl -s http://13.219.101.110:8006/api/v1/targets | jq '.data.activeTargets'
        '''
      }
    }
    stage('Health Check') {
      steps {
        script {
          def resp = sh(script: "curl -s -o /dev/null -w '%{http_code}' http://13.219.101.110:8010", returnStdout: true).trim()
          if (resp != '200') {
